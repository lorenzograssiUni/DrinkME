...
function saveSession(data) { try { sessionStorage.setItem(SESSION_KEY, JSON.stringify(data)); } catch (_) {} }
...
export default function Gioco() {
    ...
    const {
        roomCode, playerIndex, isHost: initialIsHost, players: initialPlayers,
        currentPlayerIndex: initCPI, deckCount: initDeckCount, maxPlayers,
        currentCard: initialCurrentCard, cardRevealed: initialCardRevealed,
    } = stateData ?? {};
    ...
    useEffect(() => {
        if (!roomCode) return;
        saveSession({ roomCode, playerIndex, isHost, players, currentPlayerIndex, deckCount, maxPlayers, currentCard, cardRevealed });
    }, [roomCode, playerIndex, isHost, players, currentPlayerIndex, deckCount, maxPlayers, currentCard, cardRevealed]);

    useEffect(() => {
        if (!roomCode) { navigate("/accesso", { replace: true }); return; }
        if (!socket.connected) socket.connect();
        const doRejoin = () => {
            setRejoining(true);
            socket.emit("join-room", { code: roomCode }, (res) => {
                setRejoining(false);
                if (!res?.ok) { clearSession(); navigate("/accesso", { replace: true }); }
            });
        };
        if (!location.state) {
    ...
    useEffect(() => {
        ...
        const onGameRestarted = ({ roomCode: rc, maxPlayers: mp, players: p }) => { clearSession(); const me = p.find((pl) => pl.index === playerIndex); navigate("/attesa", { replace: true, state: { roomCode: rc, playerIndex, maxPlayers: mp ?? maxPlayers, isHost: Boolean(me?.isHost) } }); };
        const onGameStateSync = ({ roomCode: rc, playerIndex: pi, maxPlayers: mp, isHost: ih, players: p, currentPlayerIndex: cpi, deckCount: dc, currentCard: cc, cardRevealed: cr }) => { setPlayers(p); setIsHost(ih); setCPI(cpi); setDeckCount(dc); setCurrentCard(cc); setCardRevealed(cr); navigate("/gioco", { replace: true, state: { roomCode: rc, playerIndex: pi, maxPlayers: mp, isHost: ih, players: p, currentPlayerIndex: cpi, deckCount: dc, currentCard: cc, cardRevealed: cr } }); };
    ...
    useEffect(() => {
        function handleOutside(e) { if (menuRef.current && !menuRef.current.contains(e.target)) setMenuAperto(false); }
        if (menuAperto) document.addEventListener("mousedown", handleOutside);
        return () => document.removeEventListener("mousedown", handleOutside);
    }, [menuAperto]);

    useEffect(() => {
        function handleKey(e) { if (e.key === "Escape") { setAiuto(false); setGiocatori(false); setMenuAperto(false); } }
        if (aiutoAperto || giocatoriAperti || menuAperto) document.addEventListener("keydown", handleKey);
        ...
        <main className="gioco-page" aria-label="Schermata di gioco">
            <section className="gioco-screen">
                    ...
                    <div className="gioco-nav-right" ref={menuRef}>
                        ...
                        {menuAperto && (
                            <div className="gioco-dropdown" role="menu">
                                <div className="gioco-dropdown-code">Codice stanza: {roomCode}</div>
                    ...
                    <div className="gioco-card-wrapper">
                        ...
                        >
                            <div className={`carta-flipper${cardRevealed ? " flipped" : ""}`}>
                                ...
                                <img src={currentCard ? cartaPath(currentCard) : backPng} alt={currentCard ? `Carta ${currentCard}` : "Carta coperta"} className="gioco-carta-img carta-back" />
                            </div>
                        </button>
                        {mazzoEsaurito && isHost && (
                            <div className="gioco-mazzo-esaurito">
                                <button className="gioco-rimescola-btn" onClick={handleRimescola}>🔀 Rimescola il mazzo</button>
                            </div>
                        )}
                        {mazzoEsaurito && !isHost && (
                            <div className="gioco-mazzo-esaurito">
                                <p className="gioco-attendi-rimescola">In attesa che l'host rimescoli...</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="gioco-player-card">
                    ...
                    <span className="gioco-player-name">{nomeAttivo.toUpperCase()}</span>
                </div>
            ...
            {giocatoriAperti && createPortal(
                <div className="aiuto-overlay" role="dialog" aria-modal="true" onClick={() => setGiocatori(false)}>
                    <div className="aiuto-popup" onClick={(e) => e.stopPropagation()}>
                        <div className="aiuto-header"><span className="aiuto-title">👥 GIOCATORI</span><button className="aiuto-close" onClick={() => setGiocatori(false)}>✕</button></div>
                        <ol className="aiuto-list">
                            {players.map((p) => (
                                <li key={p.index} className={["aiuto-item", p.index === currentPlayerIndex ? "aiuto-item--attivo" : "", p.connected === false ? "aiuto-item--disconnesso" : ""].join(" ").trim()}>
